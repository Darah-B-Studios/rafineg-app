...
const PackageDetails: React.FunctionComponent<ScreenProps<"PackageDetails">> = ({navigation}) => {
    const [isSubscribed, setIsSubscribed] = useState(false)
    return (
        <Container>
            <Appbar screenTitle="Package Details" navigation={navigation} showDrawer={false}/>
            <ScrollView style={tailwind("w-full")}>
            
                <View style={tailwind("bg-white my-3 self-center opacity-70 w-11/12")} >
                <Image
                    resizeMode='contain'
                    style={tailwind("w-full h-48")}
                    source={{uri: 'https://picsum.photos/40'}} />
                <Text style={tailwind("text-xl p-2 ")}>Package Name</Text>
                <Text style={tailwind("px-2")}>Package Details are the detailed information that one will get regarding some package he or she may be interested in subscribing for</Text>
